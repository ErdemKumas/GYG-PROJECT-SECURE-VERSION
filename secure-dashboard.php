...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_tarif'])) {
    ...
    // Kullanıcıdan alınan veriler temizleniyor
    $isim = htmlspecialchars($_POST['isim'], ENT_QUOTES, 'UTF-8');
    $malzemeler = htmlspecialchars($_POST['malzemeler'], ENT_QUOTES, 'UTF-8');
    $tarif = htmlspecialchars($_POST['tarif'], ENT_QUOTES, 'UTF-8');
    $kategori = htmlspecialchars($_POST['kategori'], ENT_QUOTES, 'UTF-8');

    // Kullanıcıya ait tarif mi veya admin mi kontrol etme kısmı
    if ($is_admin) {
        $check_owner_query = "SELECT * FROM tarifler WHERE id = ?";
        $stmt = $conn->prepare($check_owner_query);
        $stmt->bind_param("i", $tarif_id);
    } else {
        $check_owner_query = "SELECT * FROM tarifler WHERE id = ? AND username = ?";
        $stmt = $conn->prepare($check_owner_query);
        $stmt->bind_param("is", $tarif_id, $username);
    }
    ...
    if ($owner_result->num_rows > 0) {
        // Sadece kullanıcıya ait tarif veya admin güncelleyebilir
        $update_query = "UPDATE tarifler SET isim = ?, malzemeler = ?, tarif = ?, kategori = ? WHERE id = ?";
        $stmt = $conn->prepare($update_query);
        $stmt->bind_param("ssssi", $isim, $malzemeler, $tarif, $kategori, $tarif_id);
        $stmt->execute();
    }
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item">
                        <a class="nav-link" href="secure-adding-recipe.php">Yeni Tarif Ekle</a>
                    </li>
                    <li class="nav-item">
